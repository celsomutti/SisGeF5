...
uses
  ...
  cxGridTableView, cxGridDBTableView, cxGrid, cxDBNavigator, Controller.CRMContatosClientes, Controller.CRMClientes,
  Controller.CRMEnderecosClientes, Controller.CRMCNAEClientes, Controller.CRMFinanceiroClientes, Control.Estados, Common.ENum,
  Common.Utils, Controller.APICEP, Controller.APICNPJ, System.StrUtils, Control.Bancos, Vcl.DBActns;

type
  Tview_CadastroClientes = class(TForm)
    dxLayoutControl1Group_Root: TdxLayoutGroup;
    dxLayoutControl1: TdxLayoutControl;
    dxLayoutGroup1: TdxLayoutGroup;
    dxLayoutGroup2: TdxLayoutGroup;
    codigoCliente: TcxDBTextEdit;
    ...
    memTableCadastrocod_cliente: TIntegerField;
    ...
    memTableEnderecoscod_cliente: TIntegerField;
    ...
    memTableContatoscod_cliente: TIntegerField;
    ...
    memTableFinanceirocod_cliente: TIntegerField;
    ...
    memTableCNAEcod_cliente: TIntegerField;
...
var
  view_CadastroClientes: Tview_CadastroClientes;
  facao : TAcao;
  iCodigo: integer;
implementation
...
uses Data.SisGeF, View.ListaCEPs, View.ResultadoConsultaCNPJ, View.PesquisarGeral, View.PesquisaClientes;

{ Tview_CadastroEmpresas }

procedure Tview_CadastroClientes.actionConsultaCNPJExecute(Sender: TObject);
begin
  SearchCNPJ(cnpjCliente.Text);
end;

procedure Tview_CadastroClientes.actionFecharExecute(Sender: TObject);
begin
  Close;
end;

procedure Tview_CadastroClientes.actionLocalizarExecute(Sender: TObject);
begin
  SearchCliente;
end;

procedure Tview_CadastroClientes.actionPesquisarBancosExecute(Sender: TObject);
begin
  SearchBanco;
end;

procedure Tview_CadastroClientes.actionPesquisarCEPExecute(Sender: TObject);
begin
  SearchCEP(cepEndereco.Text);
end;

procedure Tview_CadastroClientes.Cancel;
begin
...
function Tview_CadastroClientes.CPFCNPJExiste(sCnpj: string): boolean;
var
  fcadastro : TCRMClientesController;
  aParam: array of variant;
begin
  try
    Result := False;
    fcadastro := TCRMClientesController.Create;
    SetLength(aParam,2);
    aParam := ['CNPJ',sCnpj];
    if fCadastro.Localizar(aParam) then
      Result := True;
  finally
    fCadastro.Clientes.Query.Active := false;
    fCadastro.Clientes.Query.Connection.Connected := False;
    fcadastro.Free;
  end;
end;

function Tview_CadastroClientes.Delete: boolean;
var
  fenderecos : TCRMEnderecosClientesController;
  fcontatos : TCRMContatosClientesController;
  ffinanceiro : TCRMFinanceiroClientesController;
  fcnae : TCRMCNAEClientesController;
begin
  try
    Result := False;
    fenderecos := TCRMEnderecosClientesController.Create;
    fcontatos := TCRMContatosClientesController.Create;
    ffinanceiro := TCRMFinanceiroClientesController.Create;
    fcnae := TCRMCNAEClientesController.Create;
    if Application.MessageBox('Confirma a exclusão deste registro ?', 'Excluir', MB_YESNO + MB_ICONQUESTION) = IDNO then
      Exit;
    fenderecos.Enderecos.Campos.Cadastro := memTableCadastrocod_cliente.AsInteger;
    ...
    fcontatos.Contatos.Campos.Cadastro := memTableCadastrocod_cliente.AsInteger;
    ...
    ffinanceiro.financeiro.Cadastro := memTableCadastrocod_cliente.AsInteger;
    ...
    fcnae.CNAE.Cadastro := memTableCadastrocod_cliente.AsInteger;
...
procedure Tview_CadastroClientes.dsCadastroStateChange(Sender: TObject);
begin
  ...
  begin
    ...
    actionFechar.Enabled := True;
    actionConsultaCNPJ.Enabled := False;
    cnpjCliente.Properties.ReadOnly := True;
    facao := tacAlterar;
  end
  ...
  begin
    ...
    cnpjCliente.Properties.ReadOnly := False;
    memTableCadastrocod_status.AsInteger := 1;
    facao := tacIncluir;
  end;
end;

procedure Tview_CadastroClientes.dsEnderecosStateChange(Sender: TObject);
begin
...
procedure Tview_CadastroClientes.EndingForm;
begin
...
procedure Tview_CadastroClientes.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  EndingForm;
  Action := caFree;
  view_CadastroClientes := nil;
end;

procedure Tview_CadastroClientes.FormShow(Sender: TObject);
begin
  StartForm;
end;

function Tview_CadastroClientes.LocateEmpresa(iID: integer): boolean;
var
  fcadastro : TCRMClientesController;
  fenderecos : TCRMEnderecosClientesController;
  fcontatos : TCRMContatosClientesController;
...
begin
  try
    ...
    begin
      ...
      memTableCadastro.CopyDataSet(fcadastro.Clientes.Query, [coRestart, coAppend]);
      fcadastro.Clientes.Query.Active := False;
      fcadastro.Clientes.Query.Connection.Connected := False;
      ...
      begin
        ...
        begin
          ...
          memTableFinanceirocod_cliente.AsInteger := ffinanceiro.financeiro.Query.FieldByName('cod_cliente').AsInteger;
...
procedure Tview_CadastroClientes.memTableCadastroAfterCancel(DataSet: TDataSet);
begin
  Cancel;
end;

procedure Tview_CadastroClientes.memTableCadastroAfterDelete(DataSet: TDataSet);
begin
  Application.MessageBox('Registro excluido com sucesso', 'Atenção', MB_OK + MB_ICONINFORMATION);
end;

procedure Tview_CadastroClientes.memTableCadastroAfterEdit(DataSet: TDataSet);
begin
  nomeRazao.SetFocus;
end;

procedure Tview_CadastroClientes.memTableCadastroAfterInsert(DataSet: TDataSet);
begin
...
procedure Tview_CadastroClientes.memTableCadastroAfterPost(DataSet: TDataSet);
begin
...
procedure Tview_CadastroClientes.memTableCadastroBeforeDelete(DataSet: TDataSet);
begin
  if not Delete then
    Abort;
end;

procedure Tview_CadastroClientes.memTableCadastroBeforePost(DataSet: TDataSet);
begin
  if memTableCadastro.Tag = -1 then Exit;
...
procedure Tview_CadastroClientes.memTableCNAEAfterInsert(DataSet: TDataSet);
begin
  memTableCNAEcod_cliente.AsInteger := memTableCadastrocod_cliente.AsInteger;
end;

procedure Tview_CadastroClientes.memTableCNAEBeforeDelete(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableCNAEBeforeEdit(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableCNAEBeforeInsert(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableContatosAfterInsert(DataSet: TDataSet);
begin
  memTableContatoscod_cliente.AsInteger := memTableCadastrocod_cliente.AsInteger;
end;

procedure Tview_CadastroClientes.memTableContatosBeforeDelete(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableContatosBeforeEdit(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableContatosBeforeInsert(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableEnderecosAfterInsert(DataSet: TDataSet);
begin
  memTableCadastrocod_cliente.AsInteger := memTableCadastrocod_cliente.AsInteger;
end;

procedure Tview_CadastroClientes.memTableEnderecosBeforeDelete(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableEnderecosBeforeEdit(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableEnderecosBeforeInsert(DataSet: TDataSet);
begin
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
    Abort;
end;

procedure Tview_CadastroClientes.memTableFinanceiroAfterInsert(DataSet: TDataSet);
begin
  memTableFinanceirocod_cliente.AsInteger := memTableCadastrocod_cliente.AsInteger;
end;

procedure Tview_CadastroClientes.memTableFinanceiroBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tview_CadastroClientes.memTableFinanceiroBeforeEdit(DataSet: TDataSet);
begin
  if memTableFinanceiro.Tag = -1 then
    Exit;
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
...
procedure Tview_CadastroClientes.memTableFinanceiroBeforeInsert(DataSet: TDataSet);
begin
  if memTableFinanceiro.Tag = -1 then
    Exit;
  if (dsCadastro.State <> dsInsert) and (dsCadastro.State <> dsEdit) then
...
procedure Tview_CadastroClientes.PopulateUF;
var
...
function Tview_CadastroClientes.Save: boolean;
var
  fcadastro : TCRMClientesController;
  fenderecos : TCRMEnderecosClientesController;
  fcontatos : TCRMContatosClientesController;
...
begin
  try
    ...
    ffinanceiro := TCRMFinanceiroClientesController.Create;
    fcnae := TCRMCNAEClientesController.Create;
    fcadastro.Clientes.Codigo := memTableCadastrocod_cliente.AsInteger;
    fcadastro.Clientes.TipoDoc := memTableCadastrodes_tipo_doc.AsString;
    fcadastro.Clientes.Nome := memTableCadastrodes_razao_social.AsString;
    fcadastro.Clientes.Alias := memTableCadastronom_fantasia.AsString;
    fcadastro.Clientes.CPF := memTableCadastronum_cnpj.AsString;
    fcadastro.Clientes.CNAE := '';
    fcadastro.Clientes.CRT := memTableCadastrocod_crt.AsInteger;
    fcadastro.Clientes.DataCadastro := memTableCadastrodat_cadastro.AsDateTime;
    fcadastro.Clientes.Status := memTableCadastrocod_status.AsInteger;
    fcadastro.Clientes.Obs := memTableCadastrodes_observacao.Text;
    fcadastro.Clientes.Acao := facao;
    ...
    iCodigo := fcadastro.Clientes.Codigo;
    if facao = tacIncluir then
    begin
      memTableCadastro.Tag := -1;
      memTableCadastro.Edit;
      memTableCadastrocod_cliente.AsInteger := iCodigo;
...
procedure Tview_CadastroClientes.SearchBanco;
var
...
procedure Tview_CadastroClientes.SearchCEP(sCEP: String);
var
...
procedure Tview_CadastroClientes.SearchCNPJ(sCNPJ: string);
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        memTableCNAEcod_cliente.AsInteger := 0;
...
procedure Tview_CadastroClientes.SearchCliente;
begin
  ...
  begin
    if LocateEmpresa(view_PesquisaClientes.fdPesquisacod_cliente.AsInteger) then
    begin
      facao := tacPesquisa;
    end;
  end;
...
procedure Tview_CadastroClientes.StartForm;
begin
  memTableCadastro.Active := True;
  memTableEnderecos.Active := True;
  memTableContatos.Active := True;
...
procedure Tview_CadastroClientes.statusPropertiesChange(Sender: TObject);
begin
...
function Tview_CadastroClientes.ValidadeBanco(sCodigo: string): String;
var
  banco : TBancosControl;
  aParam : array of variant;
  fdQuery: TFDQuery;
...
function Tview_CadastroClientes.ValidadeSave: boolean;
begin
  Result := False;
  if dscadastro.State = dsInsert then
  begin
    ...
    begin
      if not Common.Utils.TUtils.CPF(cnpjCliente.Text) then
      begin
    ...
    begin
      if not Common.Utils.TUtils.CNPJ(cnpjCliente.Text) then
      begin
    ...
    if CPFCNPJExiste(cnpjCliente.Text) then
    begin
