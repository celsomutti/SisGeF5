  ...
  try
    ...
    begin
      ...
      begin
        ...
        FPlanilha[i].CEP3 := sDetalhe[13];
        FPlanilha[i].Quantidade := StrToIntDef(sDetalhe[14], 1);
        FPlanilha[i].Volumes := StrToIntDef(sDetalhe[15],1);
        FPlanilha[i].VolumeTotal := StrToFloatDef(sDetalhe[16], 0);
        FPlanilha[i].VolumeInformado := StrToFloatDef(sDetalhe[17], 0);
        FPlanilha[i].NomeComponente := sDetalhe[18];
        FPlanilha[i].DataCriacaoRemessa :=  StrToDateDef(sDetalhe[19], 0);
        sValor := StringReplace(sDetalhe[20], '.', ',', [rfReplaceAll, rfIgnoreCase]);
        FPlanilha[i].ValorNF := StrToFloatDef(sValor, 0);
        FPlanilha[i].NorteadorPeso := sDetalhe[21];
        sValor := StringReplace(sDetalhe[22], '.', ',', [rfReplaceAll, rfIgnoreCase]);
        FPlanilha[i].ValorPagar := StrToFloatDef(sValor, 0);
        sValor := StringReplace(sDetalhe[23], '.', ',', [rfReplaceAll, rfIgnoreCase]);
        FPlanilha[i].KGM3 := StrToFloatDef(sValor, 0);
        FPlanilha[i].UnidadeMedida := sDetalhe[24];
        FPlanilha[i].NomeFornecedor := sDetalhe[25];
        FPlanilha[i].DocumentoFornecedor := sDetalhe[26];
        FPlanilha[i].Tomador := sDetalhe[27];
        FPlanilha[i].DocumentoTomador := sDetalhe[28];
        sValor := StringReplace(sDetalhe[29], '.', ',', [rfReplaceAll, rfIgnoreCase]);
        ...
        FPlanilha[i].NomeRemetente := sDetalhe[31];
        sValor := StringReplace(sDetalhe[32], '.', ',', [rfReplaceAll, rfIgnoreCase]);
