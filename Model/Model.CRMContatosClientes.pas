unit Model.CRMContatosClientes;
...
const
  ...
  TABLENAME = 'crm_clientes_contatos';

  SQLSELECT = 'select cod_cliente, seq_contato, des_contato, num_telefone, des_email ' +
              'from ' + TABLENAME + ' ';

  SQLINSERT = 'insert into ' + TABLENAME +
              '(cod_cliente, seq_contato, des_contato, num_telefone, des_email) ' +
              'values ' +
              '(:cod_cliente, :seq_contato, :des_contato, :num_telefone, :des_email);';
  ...
  SQLUPDATE = 'update ' + TABLENAME + ' set ' +
              ...
              'where ' +
              'cod_cliente = :cod_cliente and seq_contato = :seq_contato;';
...
function TCRMContatosCliente.ClearClass: Boolean;
begin
  Result := False;
...
constructor TCRMContatosCliente.Create;
begin
  FConexao := TConexao.Create;
  FContatos := TCRMContatos.Create;
end;

function TCRMContatosCliente.Delete: Boolean;
var
  sSQL: String;
  FDQuery: TFDQuery;
begin
  try
    ...
    begin
      sSQL := 'delete from ' + TABLENAME + ' ' +
              'where cod_cliente = :pcod_cliente and seq_contato = :seq_contato;';
...
function TCRMContatosCliente.GetID(iID: Integer): Integer;
var
  FDQuery: TFDQuery;
begin
  try
    FDQuery := FConexao.ReturnQuery();
    FDQuery.Open('select coalesce(max(seq_contato),0) + 1 from ' + TABLENAME + ' where cod_cliente = ' + iID.toString);
...
function TCRMContatosCliente.Gravar: Boolean;
begin
  case FAcao of
...
function TCRMContatosCliente.Insert: Boolean;
var
  FDQuery: TFDQuery;
begin
  try
...
function TCRMContatosCliente.Localizar(aParam: array of variant): Boolean;
begin
  Result := False;
  ...
  begin
    FQuery.SQL.Add('where cod_cliente = :cod_cliente and seq_contato = :seq_contato');
    FQuery.ParamByName('cod_cliente').AsInteger := aParam[1];
...
function TCRMContatosCliente.SaveBatch(memTable: TFDMemTable): Boolean;
begin
  Result := False;
...
function TCRMContatosCliente.SetupClass(FDQuery: TFDQuery): Boolean;
begin
  Result := False;
  FContatos.Cadastro := FDQuery.FieldByName('cod_cliente').AsInteger;
...
function TCRMContatosCliente.Update: Boolean;
var
  FDQuery: TFDQuery;
begin
  try
