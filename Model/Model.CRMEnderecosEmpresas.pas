unit Model.CRMEnderecosEmpresas;
...
uses Common.ENum, FireDAC.Comp.Client, Dialogs, DAO.Conexao, System.SysUtils, Data.DB, Model.CRMEnderecos;
  ...
  SQLSELECT = 'select cod_empresa, seq_endereco, des_tipo, des_logradouro, num_logradouro, des_complemento, des_bairro, ' +
...
function TCRMEnderecosEmpresas.ClearClass: boolean;
begin
  Result := False;
...
constructor TCRMEnderecosEmpresas.Create;
begin
  FConexao := TConexao.Create;
  FModelEndereco := TCRMEnderecos.Create;
end;

function TCRMEnderecosEmpresas.Delete: Boolean;
var
  FDQuery: TFDQuery;
  sSQL: String;
begin
...
function TCRMEnderecosEmpresas.GetID(iID: Integer): Integer;
var
  FDQuery: TFDQuery;
begin
  try
    FDQuery := FConexao.ReturnQuery();
    FDQuery.Open('select coalesce(max(seq_endereco),0) + 1 from ' + TABLENAME + ' where cod_empresa = ' + iID.toString);
...
function TCRMEnderecosEmpresas.Gravar: Boolean;
begin
  case FAcao of
...
function TCRMEnderecosEmpresas.Insert: Boolean;
var
  FDQuery : TFDQuery;
begin
  try
...
function TCRMEnderecosEmpresas.Localizar(aParam: array of variant): Boolean;
begin
  Result := False;
  ...
  begin
    FQuery.SQL.Add('where cod_EMPRESA = :cod_cadastro and  des_tipo = :des_tipo');
...
function TCRMEnderecosEmpresas.SaveBatch(memTable: TFDMemTable): Boolean;
 begin
  Result := False;
...
function TCRMEnderecosEmpresas.SetupClass(FDQuery: TFDQuery): boolean;
begin
  Result := False;
  FModelEndereco.Cadastro := FDQuery.FieldByName('cod_empresa').AsInteger;
  FModelEndereco.Sequencia := GetID(FDQuery.FieldByName('cod_empresa').AsInteger);
...
function TCRMEnderecosEmpresas.Update: Boolean;
var
  FDQuery: TFDQuery;
begin
  try
