...
begin
  ...
  sSQL := 'INSERT INTO ' + TABLENAME + ' ' +
          '(ID_CONTROLE, DAT_TRANSPORTE, COD_CLIENTE, DES_OPERACAO, DES_PLACA, COD_MOTORISTA, QTD_KM_SAIDA, HOR_SAIDA, ' +
          'QTD_KM_RETORNO, HOR_RETORNO, QTD_KM_RODADO, DES_SERVICO, DES_OBS, VAL_SERVICO, COD_STATUS, DES_LOG) ' +
          'VALUES (' +
          ':ID, :DATA, :CLIENTE, :OPERACAO, :PLACA, :MOTORISTA, :KMSAIDA, :HORSAIDA, :KMRETORNO, :HORRETORNO, :KMRODADO, ' +
          ':SERVICO, :OBS, :VALSERVICO, :STATUS, :LOG);';
  Connection.ExecSQL(sSQL,[aControle.ID, aControle.DataTransporte, aControle.Cliente, aControle.Operacao, aControle.Placa,
                          ...
                          aControle.Log], [ftInteger, ftDate, ftInteger, ftString, ftString, ftInteger, ftFloat, ftTime,
...
begin
  ...
  Connection.ExecSQL(sSQL,[aControle.DataTransporte, aControle.Cliente, aControle.Operacao, aControle.Placa,
                          aControle.Motorista, aControle.KmSaida, aControle.HoraSaida, aControle.KmRetorno, aControle.HoraRetorno,
                          aControle.KmRodado, aControle.Servico, aControle.Obs, aControle.ValServico, aControle.Status,
                          aControle.Log, aControle.ID], [ftDate, ftInteger, ftString, ftString, ftInteger, ftFloat, ftTime,
...
begin
  ...
  try
    ...
    begin
      Extratos.Add(TControleTransporte.Create(FDQuery.FieldByName('ID_CONTROLE').AsInteger, FDQuery.FieldByName('DAT_TRANSPORTE').AsDateTime,
